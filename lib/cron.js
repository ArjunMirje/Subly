...
export async function checkAndGenerateNotifications() {
  try {
    const { USE_MOCK_DATA } = await import('./config');
    if (USE_MOCK_DATA) {
      const { getMockSubscriptions, getMockCoupons, saveMockSubscription } = await import('./mock-db');
      const activeSubs = getMockSubscriptions().filter(s => s.status !== 'expired');
      const coupons = getMockCoupons();
      
      // Check upcoming subscription reminders
      ...
      const expiredSubs = getMockSubscriptions().filter(s => {
        if (s.status === 'expired') return false;
        const days = daysDiff(s.renewalDate);
        return days < 0;
      });
...
async function createNotificationIfNew(userId, title, message, type) {
  ...
  const { USE_MOCK_DATA } = await import('./config');
  if (USE_MOCK_DATA) {
    const { getMockNotifications, saveMockNotification } = await import('./mock-db');
    const existing = getMockNotifications().find(n => n.userId === userId && n.message === message);
    if (existing) return;
